# gprint.py
"""
gprint - A Python module for printing colored text to the terminal.
"""
...
class InvalidRGBError(Exception):
    """Exception raised when an invalid RGB color code is provided."""
    pass


# Pure Red variations (dark to light)
PURE_RED_DARK_4             = [51, 0, 0]
PURE_RED_DARK_3             = [102, 0, 0]
PURE_RED_DARK_2             = [153, 0, 0]
PURE_RED_DARK_1             = [204, 0, 0]
PURE_RED                    = [255, 0, 0]
PURE_RED_LIGHT_1            = [255, 51, 51]
PURE_RED_LIGHT_2            = [255, 102, 102]
PURE_RED_LIGHT_3            = [255, 153, 153]
PURE_RED_LIGHT_4            = [255, 204, 204]

# Orange variations (dark to light)
...
# List of all available colors for random selection
_ALL_COLORS = [
...
def get_random() -> RGBColor:
    """Return a random color from the available color palette.
    
    Returns:
        A list of 3 integers representing RGB values [R, G, B].
    """
    return choice(_ALL_COLORS)


def gprint(
    message: str,
    rgb: Union[str, RGBColor] = "default",
    new_line: bool = True,
    rainbow_mode: bool = False,
    return_me: bool = False
) -> Union[str, None]:
    """
    Print text with color support using ANSI escape codes.
    
    Args:
        message: The text to be printed.
        rgb: Color specification. Can be:
            - "default": Uses terminal default color (white text)
        ...
        new_line: If True, prints a newline after the message. Default is True.
        rainbow_mode: If True, each character is printed in a random color.
            Default is False.
        return_me: If True, returns the colorized string instead of printing.
            Useful for use with input() or other functions. Default is False.
    
    Returns:
        If return_me is True, returns the colorized string.
        Otherwise, returns None.
    
    Raises:
        InvalidRGBError: If the RGB code is invalid (not 3 values or values
            outside 0-255 range).
    
    Examples:
        >>> gprint("Hello World")  # Default color
        >>> gprint("Error!", RED)  # Using predefined color
        >>> gprint("Custom", [255, 128, 0])  # Using RGB values
        >>> gprint("Rainbow!", rainbow_mode=True)  # Rainbow text
        >>> colored = gprint("Input: ", BLUE, return_me=True)  # Get string
    """
    # Validate RGB input
    if rgb not in ["default", "RANDOM"]:
        if not isinstance(rgb, (list, tuple)) or len(rgb) != 3:
            raise InvalidRGBError("Invalid RGB code provided")
        if not all(isinstance(x, (int, str)) and 0 <= int(x) <= 255 for x in rgb):
            raise InvalidRGBError("Invalid RGB code provided")
    ...
    if rainbow_mode:
        if return_me:
            return "".join(
                gprint(char, get_random(), new_line=False, return_me=True)
                for char in message
            )
        for char in message:
    ...
    code = f"\033[38;2;{rgb[0]};{rgb[1]};{rgb[2]}m"
    colored_message = f"{code}{message}{_RESET}"
